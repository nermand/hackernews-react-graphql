...
class LinkList extends Component {
  ...
  render() {
    ...
    return (
      <div>
        {linksToRender.map((link, index) => <Link key={link.id} updateStoreAfterVote={this.updateCacheAfterVote} index={index} link={link} />)}
      </div>
    );
  }

  updateCacheAfterVote = (store, createVote, linkId) => {
    console.info('store, createVote, linkId', store, createVote, linkId);
